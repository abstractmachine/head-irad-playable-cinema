...
class _SAM3Adapter:
    ...
    Silhouette and semantic palette pipelines call
    ``segment_concept(image_pil, concept)`` with their own canonical concepts.
    """
...
def load_sam_model(project_path: str, model_name: str):
    ...
    *model_name* must be the name of a directory inside ``<project>/models/``
    ...
    is a :class:`_SAM3Adapter` exposing
...
def mask_to_polygon(mask) -> list[list[int]] | None:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with:  pip install opencv-contrib-python-headless>=4.10"
        ) from exc
