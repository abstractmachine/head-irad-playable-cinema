...
def _call_model(pipeline, messages: List[Dict[str, Any]], overrides: Optional[Dict[str, Any]] = None, images: Optional[List] = None) -> Tuple[str, str]:
    ...
    try:
        ...
        # Older APIs may not accept generation_config keyword
        try:
            outputs = model.generate(**inputs, max_new_tokens=512, do_sample=False)
        except Exception:
        ...
        # On any other error, fallback to pipeline and raise if that fails
        try:
            _txt = _fallback_text()
            res = pipeline(_txt, return_full_text=False)
            if isinstance(res, list) and res:
...
def annotate_file_shots(
    ...
    log_path = annotations_dir / f"{stem}.log"
    try:
        resolved_name = None
    ...
    # Path to video for optional frame sampling
    ...
    for i, shot in enumerate(shots):
        ...
        if scene_str is not None and str(shot.get('Scene', '')) != scene_str:
            # Keep existing annotation if present
            existing = shot.get("Shot_Caption")
            if existing:
                try:
                    results.append(json.loads(existing))
                except Exception:
        ...
        existing = shot.get("Shot_Caption")
        if existing and not force and skip_existing:
            skipped += 1
            # try to read structured JSON from existing
            try:
                parsed = json.loads(existing)
                results.append(parsed)
            except Exception:
                results.append({"shot_index": i, "note": "skipped, existing non-json caption"})
            continue
        ...
        try:
            ...
            for attempt_no, cfg in enumerate(attempt_configs, start=1):
                try:
                    full_raw, raw = _call_model(pipeline, messages, overrides=cfg, images=pil_frames if pil_frames else None)
                    last_full = full_raw
                    last_generated = raw
                    _append_log(log_path, f"SHOT {i} ATTEMPT {attempt_no} cfg={cfg}")
                    if verbose:
    ...
    # Write canonical aggregated JSON as a single file named <stem>.json
    aggregated = annotations_dir / f"{stem}.json"
    # Only create the aggregated file if there are results to write.
    try:
