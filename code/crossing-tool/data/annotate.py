...
def _is_gemma4_processor(tokenizer) -> bool:
    ...
    apply_chat_template so that <start_of_image> tokens are inserted into the
    token stream.  Calling apply_chat_template(tokenize=False) and then
    processor(images=..., text=<string>) with a Gemma processor produces zero
    image tokens in the text while the vision encoder still produces N feature
    patches, causing the 'tokens: 0, features: N' mismatch.
    """
    name = type(tokenizer).__name__
    return "Gemma" in name and "Processor" in name
...
def _call_model(pipeline, messages: List[Dict[str, Any]], overrides: Optional[Dict[str, Any]] = None, images: Optional[List] = None) -> Tuple[str, str]:
    ...
    Encoding strategy
    -----------------
    Gemma4Processor  — apply_chat_template(tokenize=True, return_dict=True,
                       images=images) in a single call so that the template
                       engine inserts <start_of_image> tokens at the same
                       time as image features are attached.
    All other models — existing two-step path: apply_chat_template(tokenize=False)
                       to get a formatted string, then processor(text=...,
                       images=...) separately (works for Qwen3-VL etc.).
    """
