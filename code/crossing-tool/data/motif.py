...
Storage schema
--------------
Motifs are stored directly inside the canonical annotation JSON:

    <project>/data/annotations/shots/<media_type>/<stem>.annotations.json

Each annotation entry's shot block contains a ``"motif"`` string field::
...
The old sidecar at ``data/motifs/`` is obsolete and must not be read or written.
...
Prompts are loaded from:
    ...
    <project>/prompts/<media_type>/motifs/
...
# ---------------------------------------------------------------------------
# Motif history helper (reads from annotation entries)
# ---------------------------------------------------------------------------
...
def load_motif_words(
    ...
    Reads ``item["shot"]["motif"]`` from the canonical annotation JSON.
...
def attach_motifs_to_annotation(*args, **kwargs):
    """OBSOLETE — motifs are now stored directly in the annotation JSON.

    This function existed to copy sidecar ``data/motifs/`` values into
    ``shot.motif`` inside the annotation JSON.  That migration is complete.
    The sidecar files are no longer read or written.
