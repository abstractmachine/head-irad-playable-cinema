...
def _make_shot_page(
    ...
    Reads motif from ``entry["shot"]["motif"]`` (canonical string) and
    combines it with palette colors.  Uses neutral fallbacks when data is absent.
    """
    shot_data = entry.get("shot", {}) if isinstance(entry, dict) else {}

    # Motif: read from canonical shot.motif field
...
def load_flipbook_data(
    ...
    # Load film-level title motif (best-effort — may be absent until generated)
    from data.film_motif import load_film_motif as _load_film_motif
    film_motif_title = _load_film_motif(project_path, filename, media_type) or {}
    ...
    for i, entry in enumerate(entries):
        ...
        # Motif: read directly from annotation entry's shot.motif
    ...
    # Film title motif from the dedicated film title file (best-effort)
    film_motif = film_motif_title
