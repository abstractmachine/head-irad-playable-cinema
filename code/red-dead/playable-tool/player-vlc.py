...
class AbstractPlayerWindow(QMainWindow):
    ...
    def load_video(self, file_path, metadata=None, timecode=None):
        ...
        # Check cooldown period - require 2 seconds between loads
        import time
        ...
        if self._vlc_loading:
            if DEBUG: print(f"DEBUG: VLC busy loading, REPLACING queued request with: {os.path.basename(file_path)}")
        ...
        if hasattr(self, 'current_video_path') and self.current_video_path == file_path:
            ...
            # Handle timecode if provided for already loaded video
        ...
        if DEBUG: print(f"DEBUG: Starting VLC load for: {os.path.basename(file_path)}")
    ...
    def _setup_vlc_events(self):
        ...
        # Add event for when media is fully loaded
    ...
    def _on_media_changed(self, event):
        """Called when VLC media changes - indicates loading is complete"""
        if DEBUG: print("DEBUG: Media changed - VLC loading complete")
        self._vlc_loading = False
        
        # Check if we have a pending request and enough time has passed
        if self._pending_load_request is not None:
            import time
            current_time = time.time()
            time_since_last_load = current_time - self._last_load_time
            
            if time_since_last_load >= 2.0:
                ...
                if DEBUG: print(f"DEBUG: Pending load request waiting for cooldown ({time_since_last_load:.1f}s < 2.0s)")
                # Keep the pending request for later processing
    ...
    def _on_duration_changed(self, event):
        ...
        if duration and duration > 0:
            ...
            # Mark loading as complete if not already done
            if self._vlc_loading:
                if DEBUG: print("DEBUG: Duration changed - VLC loading complete")
                self._vlc_loading = False
    ...
    def _on_video_playing(self, event):
        ...
        if self._vlc_loading:
            if DEBUG: print("DEBUG: Video playing - VLC loading complete")
            self._vlc_loading = False
            self._process_pending_load()
    ...
    def _process_pending_load(self):
        """Process any pending load request immediately"""
        if self._pending_load_request is not None:
            if DEBUG: print("DEBUG: Processing pending load request immediately")
            file_path, metadata, timecode = self._pending_load_request
            self._pending_load_request = None
    ...
    def _handle_pending_timecode(self):
        ...
        if self._pending_timecode is not None:
            ...
            timecode_request = self._pending_timecode
            self._pending_timecode = None  # Clear pending timecode
            
            if DEBUG: print(f"DEBUG: Pending time request: {timecode_request}")

            if ('%' in str(timecode_request)):
                duration = self.vlc_player.get_length()
                timecode = pct_to_milliseconds(timecode_request, duration)
                
                if timecode is not None:
                    if DEBUG: print(f"DEBUG: Converted {timecode_request} to milliseconds: {timecode}ms")
                    if DEBUG: print(f"DEBUG: Jumping to timecode: {milliseconds_to_timecode(timecode)}")
                    
                    # Actually jump
                    # self.set_video_time(timecode)
                else:
                    print(f"DEBUG: Invalid timecode conversion: {timecode_request} -> {timecode}ms")
        ...
        try:
            ...
            if hasattr(self, 'vlc_player') and self.vlc_player:
                ...
                if event_manager:
                    ...
                    event_manager.event_detach(vlc.EventType.MediaPlayerMediaChanged)
